materialAdmin
    ...
    .controller('materialadminCtrl', function ($timeout, $state, $scope, growlService, $rootScope, $window, $http) {

       

        //se tiver sem a session do codigo usuario redireciona p login
        if ($window.sessionStorage.getItem("codigo_usuario")<1)
        {
            $window.location.href = './index.html';        
        }

        //Welcome Message
        var nome = $window.sessionStorage.getItem("nome_usuario");
        growlService.growl('Bem Vindo ' + nome +'!', 'inverse')
    ...
    // Header
    // =========================================================================
    .controller('headerCtrl', function ($timeout,$scope, messageService, ServiceResumoFaturas) {
        ...
        ServiceResumoFaturas.getFaturado().then(function (data) {
            $scope.faturado = data[0];
            $scope.pago = data[1];
            $scope.pagoPercent = data[2];
            $scope.contestado = data[3];
            $scope.contestadoPercent = data[4];
            //alert($scope.pagoPercent);
